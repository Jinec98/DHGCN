...
from model_unsup import DHGCN_PAConv, LinearClassifier
import numpy as np
from torch.utils.data import DataLoader
from util import to_categorical, compute_overall_iou, load_cfg_from_cfg_file, merge_cfg_from_list, find_free_port
import logging
import random
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/unsup_linear.yaml', help='config file')
    parser.add_argument('opts', help='see config/unsup_linear.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def train(gpu, ngpus_per_node):
    # ============= Model ===================
    num_part = 50
    model = LinearClassifier(args, num_part)
    ...
    if args.get('pretrain', False):
        state_dict = torch.load("models/%s/pretrain_model.pth" % args.pretrain_name,
        ...
        for k in model.state_dict():
            if k.split('.')[1] == 'pretrain_model':
                f_keys.append(k)
        
        for (p_key, f_key) in zip(p_keys, f_keys):
            model.state_dict()[f_key].copy_(state_dict[p_key])
        
        for n, p in model.named_parameters():
            if n.split('.')[1] == 'pretrain_model':
        ...
        if main_process():
            ...
            logger.info(torch.load("models/%s/pretrain_model.pth" % args.pretrain_name).keys())
    else:
        if main_process():
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes):
    ...
    for batch_id, (cat_name, points, label, target, norm_plt, p2v_indices, part_distance, part_rand_idx) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        seg_pred, loss_seg = model(points, norm_plt, to_categorical(label, num_classes), p2v_indices, part_rand_idx, target)  # seg_pred: b,n,50
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        ...
        if not args.eval:  # backup the running files
            os.system('cp train_linear.py checkpoints' + '/' + args.exp_name + '/' + 'train_linear.py.backup')
