...
from model import DHGCN_PAConv
import numpy as np
...
def get_parser():
    ...
    parser.add_argument('opts', help='see config/train.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes):
    ...
    for batch_id, (cat_name, points, label, target, norm_plt, p2v_indices, part_distance, part_rand_idx) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        seg_pred, hop_logits_list, loss_seg = model(points, norm_plt, to_categorical(label, num_classes), p2v_indices, part_rand_idx, target)  # seg_pred: b,n,50
