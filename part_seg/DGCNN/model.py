...
class DHGCN_DGCNN(nn.Module):
    def __init__(self, args, class_num, cat_num):
        super(DHGCN_DGCNN, self).__init__()
        self.args = args
        self.seg_num_all = class_num
        ...
        self.conv6 = nn.Sequential(nn.Conv1d(192, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(cat_num, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x, l, p2v_indices, part_rand_idx):
        ...
        part_center, p2v_mask = get_part_feature(
            x, p2v_indices, part_rand_idx, part_num=self.args.split_num**3)
