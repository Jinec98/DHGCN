...
from model_unsup import DHGCN_DGCNN, DHGCN_AdaptConv, LinearClassifier
import numpy as np
...
def _init_():
    ...
    os.system('cp train_linear.py checkpoints'+'/' +
              args.exp_name+'/'+'train_linear.py.backup')
...
def train(args, io):
    ...
    if args.backbone == 'DGCNN':
        ...
        model_pretrain = DHGCN_AdaptConv(args).to(device)
    model_pretrain = nn.DataParallel(model_pretrain)
    
    model_cls = LinearClassifier(args, output_channels=40).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default='DHGCN_linear', type=str, metavar='N', required=True,
    ...
    parser.add_argument('--gpu', type=str, default="0", metavar='N',
                        help='Cuda id')
    # ---------------------------------------------------------
    parser.add_argument('--backbone', type=str, default='AdaptConv', choices=['AdaptConv', 'DGCNN'] , metavar='N',   
    ...
    parser.add_argument('--model_path', type=str, default='models/pretrain_model_dgcnn_mn.t7', metavar='N',
